...
<div class="box_table">
	<table class="table table-sm">
		...
		<tbody>
			...
			<tr>
				<td><?= $no ?></td>
				<td><?= $data['kode_salad'] ?></td>
				<td><?= $data['nama_salad'] ?></td>
				<td><?= $data['harga_salad'] ?></td>
				<td><?= $data['stok_salad'] ?></td>
				<td><?= $data['jenis_salad'] ?></td>
				<td><?= $data['deskripsi_salad'] ?></td>
				<td>
					<a href="<?= $WEB_CONFIG['base_url'] ?>index.php?page=update&id=<?= $data['kode_salad'] ?>" class="btn btn-success m-1">
					...
					<a href="javascript:alertDelete('<?= $data['kode_salad'] ?>');" class="btn btn-danger m1">
					<svg style="width:20px;height:20px" viewBox="0 0 24 24" class="mb-1">
